...
class Position extends REST_Controller {
    ...
    {
        parent::__construct();
		$this->load->model('position_model', 'the_model');
    }
	...
	function delete_post()
	{
        ...
        $id = filter($this->post('id_position'));
        ...
        if ($validation == "ok")
		{
            $query = $this->the_model->info(array('id_position' => $id));
	...
	function lists_get()
	{
		...
		if (in_array($order, $this->config->item('default_position_order')) && ($order == TRUE))
		{
			$order = $order;
		}
		else
		...
		if (in_array($status, $this->config->item('default_position_status')) && ($status == TRUE))
		{
			$status = $status;
		}
