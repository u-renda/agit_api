...
class Project_type extends REST_Controller {

    function __construct()
    {
        parent::__construct();
		$this->load->model('project_type_model', 'the_model');
    }
	...
	function delete_post()
	{
        ...
        $id = filter($this->post('id_project_type'));
        ...
        if ($validation == "ok")
		{
			...
			{
				$data['id_project_type'] = 'not found';
				$validation = "error";
				$code = 400;
			}
		}
	...
	function lists_get()
	{
		...
		if (in_array($order, $this->config->item('default_project_type_order')) && ($order == TRUE))
		{
			$order = $order;
		}
		else
