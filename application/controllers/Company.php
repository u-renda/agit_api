...
class Company extends REST_Controller {
    ...
    {
        parent::__construct();
		$this->load->model('company_model', 'the_model');
    }
	...
	function delete_post()
	{
        ...
        $id = filter($this->post('id_company'));
        ...
        if ($validation == "ok")
		{
            $query = $this->the_model->info(array('id_company' => $id));
	...
	function lists_get()
	{
		...
		if (in_array($order, $this->config->item('default_company_order')) && ($order == TRUE))
		{
			$order = $order;
		}
		else
		...
		if (in_array($status, $this->config->item('default_company_status')) && ($status == TRUE))
		{
			$status = $status;
		}
